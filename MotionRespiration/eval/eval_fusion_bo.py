...
#   - 新增：仅用于画图的 频域带通 + 时间平滑 + 互相关相位对齐（不影响 RR 计算）
...
def smooth_for_plot(
    ...
    rr_min_bpm: float,
    rr_max_bpm: float,
    ma_win_sec: float = 0.7,
) -> np.ndarray:
    """
    仅用于画图的平滑：
      1) FFT 频域带通，只保留 [rr_min, rr_max] bpm 对应频率
      2) 时域滑动平均（窗口约 ma_win_sec 秒）
    不参与训练 / 不参与 RR 计算，只是让 GT/Pred 波形更平滑、毛刺更少。
    """
    ...
    # 2) 频域带通
    N = len(x0)
    freqs = np.fft.rfftfreq(N, d=1.0 / fps)  # Hz
    X = np.fft.rfft(x0)

    f_min = rr_min_bpm / 60.0  # bpm -> Hz
    f_max = rr_max_bpm / 60.0
    mask = (freqs >= f_min) & (freqs <= f_max)
    X_filt = X * mask
    x_bp = np.fft.irfft(X_filt, n=N)

    # 3) 滑动平均（时间域再平滑一次，去掉残余毛刺）
    win = int(ma_win_sec * fps)
    if win < 1:
        win = 1
    kernel = np.ones(win, dtype=np.float32) / float(win)
    x_smooth = np.convolve(x_bp, kernel, mode="same")
...
def align_by_xcorr(
    ...
    """
    用互相关把 sig 在时间轴上对齐到 ref：
      ref: 参考信号（这里用 GT）
      sig: 需要对齐的信号（这里用 Pred）
    返回:
      sig_aligned: 对齐后的 sig
      best_lag:    以“样本点”为单位的平移量（>0 表示原 sig 落后于 ref）
    """
    if ref.ndim != 1 or sig.ndim != 1:
        return sig.copy(), 0
    ...
    corr = np.correlate(ref0, sig0, mode="full")  # 长度 2N-1
    lags = np.arange(-N + 1, N)

    # 限制最大允许平移（例如 3 秒内）
    if max_shift_sec is not None and max_shift_sec > 0:
        max_shift = int(max_shift_sec * fps)
        valid = (lags >= -max_shift) & (lags <= max_shift)
        corr = corr[valid]
        lags = lags[valid]

    best_lag = int(lags[np.argmax(corr)])  # ref 和 sig 的最佳对齐滞后量
    # 为了把 sig 对齐到 ref，把 sig 整体反向移动 best_lag
    sig_aligned = np.roll(sig, -best_lag)

    return sig_aligned, best_lag
...
def plot_waveform_pair(
    ...
    - rr_min_bpm, rr_max_bpm: 用来做频域带通的 RR 频带
    """
    ...
    # 1) 先做频域带通 + 时间平滑
    gt_smooth = smooth_for_plot(gt, fps=fps, rr_min_bpm=rr_min_bpm, rr_max_bpm=rr_max_bpm)
    pred_smooth = smooth_for_plot(pred, fps=fps, rr_min_bpm=rr_min_bpm, rr_max_bpm=rr_max_bpm)

    # 2) 再用互相关在时间轴上对齐 Pred 到 GT，减少相位差
    pred_aligned, best_lag = align_by_xcorr(
        ref=gt_smooth,
        sig=pred_smooth,
        fps=fps,
        max_shift_sec=3.0,  # 最多允许 ±3 秒的对齐
    )
    # 你如果想看 shift，可以打开这一行：
    # print(f"[EvalFusion] sample {sample_idx:03d} xcorr shift = {best_lag} frames ({best_lag / fps:.2f} s)")

    # 3) 最后做 z-score 方便形状对比
    gt_z = _zscore(gt_smooth)
    pred_z = _zscore(pred_aligned)

    t_axis = np.arange(T) / fps  # 以秒为横坐标

    plt.figure(figsize=(8, 3))
    plt.plot(t_axis, gt_z, label="GT (z-score, smoothed)", linewidth=1.0)
    plt.plot(t_axis, pred_z, label="Pred (z-score, smoothed + aligned)", linewidth=1.0, linestyle="--")
    plt.xlabel("Time (s)")
    ...
    for idx, (rgb_clip, flow_clip, resp) in enumerate(loader):
        ...
        if plotted < MAX_PLOTS:
            plot_waveform_pair(
                ...
                pred=pred_resp_np,
                sample_idx=idx,
                rr_min_bpm=rr_min_eff,
                rr_max_bpm=rr_max_eff,
                save_dir=PLOTS_DIR,
                fps=FPS,
            )
        plotted += 1
